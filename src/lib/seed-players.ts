...
// Players sorted by odds (best odds = lowest odds_value = Group 1)
// Grouped 7 per group (8 groups = 56 selectable players)
// Odds sourced from easyofficepools.com Masters odds rankings
const PLAYERS = [
  // Group 1: Odds ranks 1-7 (top favorites)
  ...
  // Group 8: Past champions & remaining field (longshots)
...
export function seedPlayers() {
  ...
  const seedAll = db.transaction(() => {
    PLAYERS.forEach((p, i) => {
      insert.run(p.name, p.odds, p.odds_value, i + 1, p.group_number, p.world_rank);
    });
  });
